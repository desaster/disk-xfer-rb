  ...
  while (wait_time>0)
    {
      delay(1);
      wait_time--;
      if (rbcomms_data_waiting() != 0) {
        if (rbcomms_read_byte() == 'C') {
          state = BLOCK;
...
void xmodem_state_block(void)
{
  ...
  if (rbdisk_read_sector(cylinder,head,sector,buf)==0)
    {
      printf("Sending Cylinder: %4d, Head: %2d, Sector: %2d...",cylinder,head,sector);

      rbcomms_send_byte(0x01);  // SOH
      rbcomms_send_byte(block_num); // block # (mod 256)
      rbcomms_send_byte(0xFF-block_num); // 0xFF - BLOCK # (simple checksum)

      for (i=0;i<512;i++)     // Send the data
        rbcomms_send_byte(buf[i]);

      calced_crc=xmodem_calc_crc(buf,512);
      rbcomms_send_byte((calced_crc>>8));       // CRC Hi
      rbcomms_send_byte(calced_crc&0xFF);       // CRC Lo
...
void xmodem_set_next_sector(void)
{
  if (sector>=geometry.s)
    {
      sector=1;
      if (head >= geometry.h - 1) {
        head = 0;
        if (cylinder >= geometry.c - 1)
              state = END;
        else
              cylinder++;
      } else
